...
- **viewSource** – optional mapping of a table field to another table and view.
  Each entry defines `{ "table": "TABLE", "view": "VIEW_TABLE" }`. The field value
  is used as the `WHERE` parameter when loading data from the view. Additional
  columns from that view are displayed in the form but are not saved back to the
  transaction table.
