...
- **defaultVariation** and **variationDefaults** – identify the default variation key and any per-variation field defaults (such as `type` and `taxType`). Only defaults for the selected variation are applied.
