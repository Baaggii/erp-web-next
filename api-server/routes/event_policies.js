...
router.get('/:id(\d+)', async (req, res, next) => {
  try {
    ...
    if (typeof graphJson === 'string') {
      try { graphJson = JSON.parse(graphJson); } catch {}
    }
    res.json({ ...row, condition_json: conditionJson, action_json: actionJson, graph_json: graphJson });
  } catch (error) {
    next(error);
  }
});
...
router.get('/scenarios', async (req, res, next) => {
  try {
    ...
    const scenarios = rows.map((row) => ({
      ...
      default_graph_json: parseJsonSafely(row.default_graph_json, null),
    }));
    res.json(scenarios);
  } catch (error) {
    next(error);
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      (policy_key, policy_name, event_type, module_key, priority, is_active, stop_on_match, condition_json, action_json, graph_json, ai_policy_json, company_id, created_by, updated_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        payload.policy_key,
        payload.policy_name,
        ...
        payload.graph_json ? JSON.stringify(payload.graph_json) : null,
    ...
    await pool.query(
      `UPDATE core_event_policies
       SET policy_name = ?, module_key = ?, event_type = ?, priority = ?, is_active = ?, stop_on_match = ?,
           condition_json = ?, action_json = ?, graph_json = ?, ai_policy_json = ?, updated_by = ?, updated_at = NOW()
...
router.post('/drafts', async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      (company_id, policy_name, policy_key, event_type, module_key, priority, is_active, condition_json, action_json, graph_json, created_by, updated_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        req.user.companyId,
        payload.policy_name,
...
router.put('/drafts/:id', async (req, res, next) => {
  try {
    ...
    await pool.query(
      ...
      condition_json=?, action_json=?, graph_json=?, updated_by=?, updated_at=NOW()
...
router.post('/deploy/:draftId', async (req, res, next) => {
  ...
  try {
    ...
    const validation = validateEventPolicySchema({ condition_json: parseJsonSafely(draft.condition_json, {}), action_json: parseJsonSafely(draft.action_json, {}), graph_json: parseJsonSafely(draft.graph_json, null) });
    if (!validation.ok) {
      await conn.rollback();
    ...
    if (policyId) {
      await conn.query(
        `UPDATE core_event_policies SET policy_name=?, event_type=?, module_key=?, priority=?, is_active=?,
         condition_json=?, action_json=?, graph_json=?, updated_by=?, updated_at=NOW() WHERE policy_id=? AND company_id=?`,
        [draft.policy_name, draft.event_type, draft.module_key, draft.priority, draft.is_active, draft.condition_json, draft.action_json, draft.graph_json, req.user.empid, policyId, req.user.companyId],
      );
    } else {
      const [insertResult] = await conn.query(
        `INSERT INTO core_event_policies
        (policy_key, policy_name, event_type, module_key, priority, is_active, stop_on_match, condition_json, action_json, graph_json, company_id, created_by, updated_by)
        VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)`,
        [draft.policy_key, draft.policy_name, draft.event_type, draft.module_key, draft.priority, draft.is_active, draft.condition_json, draft.action_json, draft.graph_json, req.user.companyId, req.user.empid, req.user.empid],
      );
    ...
    await conn.query(
      `INSERT INTO policy_versions
       (policy_id, condition_json, action_json, graph_json, version_number, created_by, company_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [policyId, draft.condition_json, draft.action_json, draft.graph_json, nextVersion, req.user.empid, req.user.companyId],
    );
