...
import { processPendingEvents, processEventById } from '../services/eventProcessorService.js';
...
router.post('/replay/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const result = await processEventById({ eventId: req.params.id, companyId: req.user.companyId });
    res.json(result);
  } catch (error) {
    next(error);
  }
