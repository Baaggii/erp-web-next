...
export function createPeriodControlRouter(deps = {}) {
  ...
  router.get('/preview-result', authMiddleware, async (req, res) => {
    const companyId = Number(req.query.company_id || req.user.companyId);
    const fiscalYear = Number(req.query.fiscal_year || new Date().getFullYear());
    const procedureName = String(req.query.procedure_name || '').trim();
    ...
    try {
      ...
      const results = await previewReports({
        companyId,
        fiscalYear,
        reportProcedures: [procedureName],
      });
      const result = Array.isArray(results) ? results[0] : null;
      if (!result) return res.status(404).json({ ok: false, message: 'Preview result not found' });
      return res.json({ ok: true, result });
    } catch (error) {
      return res.status(500).json({ ok: false, message: error?.message || 'Failed to load preview result' });
    }
  });
