  ...
  try {
    ...
    if (Array.isArray(columns)) {
      columns.forEach((col) => {
        ...
        if (!trimmed) return;
        columnNameLookup.set(trimmed.toLowerCase(), trimmed);
      });
    }
    ...
    const applyFieldValue = (columns, value) => {
      if (value === undefined || value === null) return;
      if (typeof value === 'string' && value.trim() === '') return;
      columns.forEach((column) => {
        sanitizedValues[column] = value;
      });
    };
    ...
    applyFieldValue(resolvedBranchFields, reviewerBranchId ?? row.branch_id ?? null);
    ...
    applyFieldValue(
      ...
      reviewerDepartmentId ?? row.department_id ?? null,
    );
