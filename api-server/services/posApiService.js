...
export async function getPosApiToken() {
  ...
  const { config, missing } = await resolvePosApiConfig(requiredEnv);
  const resolvedMissing = [...missing];
  ...
  if (!clientSecret && !resolvedMissing.includes('POSAPI_CLIENT_SECRET')) {
    resolvedMissing.push('POSAPI_CLIENT_SECRET');
  }
