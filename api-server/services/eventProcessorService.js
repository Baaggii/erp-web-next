...
async function processEventRow(row, conn) {
  ...
  try {
    ...
    return { eventId: event.eventId, status: finalStatus, ignored: !matchedAny };
  } catch (error) {
    await conn.query(
      `UPDATE core_events
    ...
    return { eventId: event.eventId, status: 'failed', error: error?.message };
  }
}

function buildSummary(results = []) {
  return {
    processed: results.filter((r) => r.status === 'processed' || r.status === 'ignored').length,
...
export async function processPendingEvents({ companyId, limit = 50, conn = pool } = {}) {
  ...
  for (const row of events) {
    results.push(await processEventRow(row, conn));
  }
  return buildSummary(results);
}

export async function processEventById({ eventId, companyId, conn = pool } = {}) {
  const [rows] = await conn.query(
    `SELECT * FROM core_events
      WHERE event_id = ? AND company_id = ? AND deleted_at IS NULL
      LIMIT 1`,
    [eventId, companyId],
  );
  if (!rows.length) {
  ...
  const result = await processEventRow(rows[0], conn);
