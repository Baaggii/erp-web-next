...
let ioRef = null;
const onlineByCompany = new Map(); // Map<companyId, Map<empid, Set<socketId>>>
...
function listOnline(companyId) {
  const companyMap = onlineByCompany.get(companyId);
...
export async function listMessages({ user, companyId, limit = 250 }) {
  ...
  const batchSize = 500;
  const maxScanRows = Math.max(safeLimit * 8, 2000);
  const allRows = [];
  let offset = 0;
  while (allRows.length < maxScanRows) {
    const [batch] = await pool.query(
      `SELECT *
        ...
        LIMIT ? OFFSET ?`,
      [scopedCompanyId, batchSize, offset],
    );
  ...
  const limitedVisibleNodes = visibleNodes.slice(-safeLimit);
...
export async function createMessage({ user, payload, companyId }) {
  ...
  if (ioRef) {
    const emitTargets = new Set([user.empid, ...recipients]);
    if (parentMessageId) {
      const threadParticipants = await resolveThreadParticipants(parentMessageId);
      threadParticipants.forEach((empid) => emitTargets.add(empid));
    }
    emitTargets.forEach((empid) => {
      ioRef.to(`emp:${empid}`).emit('messages:new', created);
    });
  }
...
export function markOnline(companyId, empid, socketId = null) {
  const scopedCompanyId = toId(companyId);
  const normalizedEmpid = normalizeEmpid(empid);
  if (!scopedCompanyId || !normalizedEmpid) return;
  if (!onlineByCompany.has(scopedCompanyId)) onlineByCompany.set(scopedCompanyId, new Map());
  const companyMap = onlineByCompany.get(scopedCompanyId);
  if (!companyMap.has(normalizedEmpid)) companyMap.set(normalizedEmpid, new Set());
  const socketSet = companyMap.get(normalizedEmpid);
  socketSet.add(String(socketId || normalizedEmpid));
  if (ioRef) {
...
export function markOffline(companyId, empid, socketId = null) {
  const scopedCompanyId = toId(companyId);
  const normalizedEmpid = normalizeEmpid(empid);
  if (!scopedCompanyId || !normalizedEmpid) return;
  const companyMap = onlineByCompany.get(scopedCompanyId);
  ...
  if (socketId) {
    socketSet.delete(String(socketId));
  } else {
    socketSet.clear();
  }

  if (socketSet.size === 0) {
    companyMap.delete(normalizedEmpid);
  }
  if (companyMap.size === 0) {
    onlineByCompany.delete(scopedCompanyId);
  }
  if (ioRef) {
    ioRef.to(`company:${scopedCompanyId}`).emit('messages:presence', {
      companyId: scopedCompanyId,
