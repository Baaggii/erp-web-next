...
async function callProcedureSafely(action, companyId, conn = pool) {
  ...
  const includeCompanyScope = action?.includeCompanyScope !== false;
  const callParams = includeCompanyScope ? [...params, companyId] : [...params];
  const placeholders = callParams.map(() => '?').join(', ');
  const sql = `CALL \`${procedure}\`(${placeholders})`;
  await conn.query(sql, callParams);
  return { called: procedure, includeCompanyScope };
}
