...
async function buildLineageFromConfig(config, companyId) {
  ...
  for (const field of config.fields) {
    ...
    if (field.source === 'field' && field.table && field.field) {
      ...
      const resolved = resolveFromAlias(field.baseAlias);
      if (resolved) {
        entry.sourceTable = resolved.sourceTable;
        entry.sourceColumn = resolved.sourceColumn;
        entry.expr = resolved.expr;
      }
    } else if (Array.isArray(field.calcParts) && field.calcParts.length === 1) {
      const part = field.calcParts[0];
      if (part?.source === 'field' && part.table && part.field) {
        entry.sourceTable = part.table;
        entry.sourceColumn = part.field;
        entry.expr = `${part.table}.${part.field}`;
      } else if (part?.source === 'alias' && part.alias) {
        const resolved = resolveFromAlias(part.alias);
        if (resolved) {
          entry.sourceTable = resolved.sourceTable;
          entry.sourceColumn = resolved.sourceColumn;
          entry.expr = resolved.expr;
