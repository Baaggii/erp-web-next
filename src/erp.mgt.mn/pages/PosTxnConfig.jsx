...
export default function PosTxnConfig() {
  ...
  const endpointReceiptItemsEnabled = endpointSupportsItems
    ...
    : selectedEndpoint
      ...
      : false;
  const endpointReceiptTypesEnabled = endpointSupportsItems
    ? true
    : selectedEndpoint
      ? selectedEndpoint.enableReceiptTypes !== false
      : true;
  const endpointReceiptTaxTypesEnabled = endpointSupportsItems
    ? true
    : selectedEndpoint
      ? selectedEndpoint.enableReceiptTaxTypes !== false
      : true;
  const endpointPaymentMethodsEnabled = endpointSupportsItems
    ? true
    : selectedEndpoint
    ...
    () =>
      selectedEndpoint
        ? buildEndpointRequestMappingDefaults(selectedEndpoint, {
            nestedPaths: endpointNestedPaths,
            supportsItems: endpointSupportsItems,
          })
        : null,
    [endpointNestedPaths, endpointSupportsItems, selectedEndpoint],
  );
  ...
  useEffect(() => {
    ...
    getTransactionForms()
      .then((data) => {
        const { byTable, names, mapping, fields } = buildTransactionFormIndex(data);
