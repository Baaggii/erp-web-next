...
function CncProcessingPage() {
  ...
  async function handleSubmit(event) {
    ...
    const buildFormData = () => {
      ...
      payload.append('conversionType', processingType);
    ...
    try {
      ...
      const sendCncRequest = async (url, attemptIndex) => {
        ...
        addApiLog({
          name: `CNC processing request (attempt ${attemptIndex + 1})`,
      ...
      let attempt;
      for (let index = 0; index < cncEndpoints.length; index += 1) {
        if (index > 0) {
          addStep(
            'Retrying CNC processing with alternate endpoint',
            'success',
            `Retrying ${cncEndpoints[index]}`,
          );
        }
        attempt = await sendCncRequest(cncEndpoints[index], index);
