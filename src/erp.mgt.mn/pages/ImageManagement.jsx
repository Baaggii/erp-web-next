...
// IndexedDB helpers for storing directory handles and large session data
function getHandleDB() {
  if (typeof indexedDB === 'undefined') return Promise.reject();
  return new Promise((resolve, reject) => {
    const req = indexedDB.open('imgMgmtHandles', 2);
    req.onupgradeneeded = () => {
      const db = req.result;
...
async function saveSessionData(key, data) {
  try {
    const db = await getHandleDB();
    await new Promise((res, rej) => {
...
export default function ImageManagement() {
  ...
  async function loadTables({ silent = false } = {}) {
    try {
      let data = await loadSessionData(FOLDER_STATE_KEY);
      if (!data) {
  ...
  function persistSnapshot(partial = {}) {
    try {
      ...
      saveSessionData(FOLDER_STATE_KEY, data);
  ...
  async function saveTables() {
    try {
      ...
      const ok = await saveSessionData(FOLDER_STATE_KEY, data);
