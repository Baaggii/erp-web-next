...
export default function PosApiAdmin() {
  ...
  const [infoSyncUsage, setInfoSyncUsage] = useState('info');
  ...
  async function handleManualSync() {
    try {
      ...
      const res = await fetch(`${API_BASE}/posapi/reference-codes/sync`, {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ usage: infoSyncUsage, endpoints: infoSyncEndpointIds }),
      });
      if (!res.ok) {
              ...
              <div style={styles.infoGrid}>
                ...
                <label style={{ ...styles.label, minWidth: '240px' }}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setInfoSyncEndpointIds(Array.from(e.target.selectedOptions).map((opt) => opt.value))
                    }
