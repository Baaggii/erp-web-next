...
export default function PosApiAdmin() {
  ...
  function syncRequestSampleToVariation(key) {
    if (!key) return {};
    const baseSource = parseExamplePayload(
      formState.requestSampleText || formState.requestSchemaText || BASE_COMPLEX_REQUEST_SCHEMA,
    );
    const basePayload = baseSource && typeof baseSource === 'object'
      ? JSON.parse(JSON.stringify(baseSource))
      : {};
    const variationPayload = getVariationExamplePayload(key);
    let hydratedPayload = mergePayloads(basePayload, variationPayload);
    if (variationPayload && typeof variationPayload === 'object') {
      try {
        const pretty = JSON.stringify(hydratedPayload, null, 2);
    ...
    hydratedPayload = syncRequestSampleFromSelections(selections, hydratedPayload) || hydratedPayload;
    setFormState((prev) => ({
      ...prev,
  ...
  async function handleTest(payloadOverride) {
    ...
    if (!payloadForTest && selectedVariationKey) {
      payloadForTest = syncRequestSampleToVariation(selectedVariationKey);
    }
