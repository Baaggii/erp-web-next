...
function getValueFromTokens(source, tokens) {
  ...
  for (const token of tokens) {
    ...
    if (typeof current === 'string') current = parseMaybeJson(current);
...
function extractValueFromPath(source, path) {
...
const EMPTY_LOOKUP_STATE = Object.freeze({
  params: {},
  fieldBindings: {},
  bodyText: '',
  response: null,
...
function cloneLookupState(state = EMPTY_LOOKUP_STATE) {
  return {
    ...state,
    params: { ...(state?.params || {}) },
    fieldBindings: { ...(state?.fieldBindings || {}) },
    mappedFields: Array.isArray(state?.mappedFields) ? [...state.mappedFields] : [],
  };
}

function formatLookupResponse(value) {
...
const RowFormModal = function RowFormModal({
  ...
  const columnOptions = React.useMemo(
    () => columns.map((col) => ({ value: col, label: labels?.[col] || col })),
    [columnsKey, labels],
  );
  const infoEndpointList = React.useMemo(
    () =>
      Array.from(
        new Set(
          (Array.isArray(infoEndpoints) ? infoEndpoints : [])
  ...
  const infoEndpointListKey = React.useMemo(
    () => infoEndpointList.join('|'),
    [infoEndpointList],
  );
  const infoEndpointMappingsKey = React.useMemo(
    () => JSON.stringify(infoEndpointMappings || {}),
    [infoEndpointMappings],
  );
  const infoEndpointMappingsLookup = React.useMemo(() => {
    const map = {};
    if (!infoEndpointMappings || typeof infoEndpointMappings !== 'object') return map;
    Object.entries(infoEndpointMappings).forEach(([endpointId, mapping]) => {
      ...
      Object.entries(mapping).forEach(([path, column]) => {
        ...
        sanitized[trimmedPath] = trimmedColumn;
      });
      if (Object.keys(sanitized).length > 0) map[endpointId] = sanitized;
    });
    return map;
  }, [infoEndpointMappingsKey]);
  const [infoEndpointDefinitions, setInfoEndpointDefinitions] = useState([]);
  const [infoEndpointsLoading, setInfoEndpointsLoading] = useState(false);
  const [selectedInfoEndpointId, setSelectedInfoEndpointId] = useState('');
  const [infoLookupStateMap, setInfoLookupStateMap] = useState({});
  const updateLookupState = React.useCallback((endpointId, updater) => {
    if (!endpointId) return;
    setInfoLookupStateMap((prev) => {
      const existing = prev[endpointId];
      const base = existing ? cloneLookupState(existing) : createEmptyLookupState();
      const nextState =
        typeof updater === 'function' ? updater(base) : { ...base, ...(updater || {}) };
      return { ...prev, [endpointId]: nextState };
    });
  }, []);
  const updateParamValue = React.useCallback(
    (endpointId, paramName, value) => {
      if (!endpointId || !paramName) return;
      updateLookupState(endpointId, (state) => ({
        ...state,
  ...
  const updateParamBinding = React.useCallback(
    (endpointId, paramName, columnName) => {
      if (!endpointId || !paramName) return;
      updateLookupState(endpointId, (state) => {
        const nextBindings = { ...(state.fieldBindings || {}) };
        const nextParams = { ...(state.params || {}) };
        if (!columnName) {
          delete nextBindings[paramName];
        } else {
          nextBindings[paramName] = columnName;
          const lower = columnName.toLowerCase();
          const matched = columns.find((col) => col.toLowerCase() === lower);
          if (matched) {
            nextParams[paramName] = formValsRef.current?.[matched] ?? '';
          }
        }
        return { ...state, fieldBindings: nextBindings, params: nextParams };
      });
    },
    [updateLookupState, columns],
  );
  const updateBodyText = React.useCallback(
    (endpointId, text) => {
      if (!endpointId) return;
      updateLookupState(endpointId, (state) => ({ ...state, bodyText: text }));
    },
    [updateLookupState],
  );
  const applyInfoEndpointMapping = React.useCallback(
    (endpointId, response) => {
      const mapping = infoEndpointMappingsLookup[endpointId];
      if (!mapping) return [];
      const updates = {};
      Object.entries(mapping).forEach(([path, columnName]) => {
        const value = extractValueFromPath(response, path);
        if (value === undefined) return;
        const lower = columnName.toLowerCase();
        if (!columnLowerSet.has(lower)) return;
        const actualColumn = columns.find((col) => col.toLowerCase() === lower) || columnName;
        const normalized = normalizeLookupValue(value);
        if (valuesEqual(formValsRef.current?.[actualColumn], normalized)) return;
        updates[actualColumn] = normalized;
      });
      if (Object.keys(updates).length === 0) return [];
      const result = setFormValuesWithGenerated((prev) => ({ ...prev, ...updates })) || {};
      const mappedEntries = Object.entries(result.diff || {}).map(([column, value]) => ({
        column,
        value,
      }));
      return mappedEntries;
    },
    [infoEndpointMappingsLookup, columnLowerSet, columns, setFormValuesWithGenerated],
  );
  const handleInvokeInfoEndpoint = React.useCallback(async () => {
    if (!selectedInfoEndpointId) return;
    const definition = infoEndpointDefinitions.find((def) => def?.id === selectedInfoEndpointId);
    if (!definition) return;
    const state = infoLookupStateMap[selectedInfoEndpointId] || EMPTY_LOOKUP_STATE;
    ...
    if (method !== 'GET' && method !== 'HEAD') {
      ...
      if (text.trim()) {
        try {
          ...
          updateLookupState(selectedInfoEndpointId, (prevState) => ({
            ...prevState,
    ...
    try {
      const res = await fetch('/api/posapi/info/invoke', {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          endpointId: selectedInfoEndpointId,
          params,
          body: parsedBody,
          table,
          formName,
      ...
      if (!res.ok) {
        ...
        const message = errorData?.message || `Lookup failed (${res.status})`;
        updateLookupState(selectedInfoEndpointId, (prevState) => ({
          ...prevState,
          loading: false,
          error: message,
      ...
      const mapped = applyInfoEndpointMapping(selectedInfoEndpointId, responseData);
      updateLookupState(selectedInfoEndpointId, (prevState) => ({
        ...prevState,
        loading: false,
    ...
    selectedInfoEndpointId,
    infoEndpointDefinitions,
    infoLookupStateMap,
    updateLookupState,
    table,
    formName,
    applyInfoEndpointMapping,
  ]);
  useEffect(() => {
    setInfoLookupStateMap((prev) => {
      const allowed = new Set(infoEndpointList);
      let changed = false;
      const next = {};
      Object.entries(prev).forEach(([endpointId, state]) => {
        if (!allowed.has(endpointId)) {
          changed = true;
          return;
        }
        next[endpointId] = state;
      });
      if (!changed && Object.keys(next).length === Object.keys(prev).length) return prev;
      return next;
    });
    if (selectedInfoEndpointId && !infoEndpointList.includes(selectedInfoEndpointId)) {
      setSelectedInfoEndpointId('');
    }
  }, [infoEndpointListKey, infoEndpointList, selectedInfoEndpointId]);
  useEffect(() => {
    if (infoEndpointList.length === 0) {
      setInfoEndpointDefinitions([]);
      setSelectedInfoEndpointId('');
      setInfoEndpointsLoading(false);
      return;
    }
    let cancelled = false;
    const controller = new AbortController();
    setInfoEndpointsLoading(true);
    const params = new URLSearchParams();
    infoEndpointList.forEach((id) => params.append('ids', id));
    fetch(`/api/posapi/info?${params.toString()}`, {
      credentials: 'include',
      signal: controller.signal,
    })
      .then((res) => {
        if (!res.ok) throw new Error('Failed to load POSAPI info endpoints');
        return res.json();
      })
      .then((data) => {
        if (cancelled) return;
        const endpoints = Array.isArray(data?.endpoints)
          ...
          : Array.isArray(data)
            ? data
            : [];
        const sanitized = endpoints.filter((endpoint) => endpoint && endpoint.id);
        setInfoEndpointDefinitions(sanitized);
      })
      .catch((err) => {
  ...
  }, [infoEndpointListKey, infoEndpointList]);
  useEffect(() => {
    if (!infoEndpointDefinitions.length) {
      if (selectedInfoEndpointId) setSelectedInfoEndpointId('');
      return;
    }
    const exists = infoEndpointDefinitions.some((def) => def?.id === selectedInfoEndpointId);
    if (!exists) {
      setSelectedInfoEndpointId(infoEndpointDefinitions[0].id);
    }
  }, [infoEndpointDefinitions, selectedInfoEndpointId]);
  useEffect(() => {
    if (!selectedInfoEndpointId) return;
    const state = infoLookupStateMap[selectedInfoEndpointId];
    if (!state) return;
    const bindings = state.fieldBindings || {};
    const entries = Object.entries(bindings);
    if (entries.length === 0) return;
    const nextParams = { ...(state.params || {}) };
    let changed = false;
    entries.forEach(([paramName, columnName]) => {
      if (!paramName || !columnName) return;
      const lower = columnName.toLowerCase();
      const matched = columns.find((col) => col.toLowerCase() === lower);
      if (!matched) return;
      const value = formVals[matched];
      if (!valuesEqual(nextParams[paramName], value)) {
  ...
  const selectedDefinition = infoEndpointDefinitions.find(
    (def) => def?.id === selectedInfoEndpointId,
  );
  const currentLookupState = infoLookupStateMap[selectedInfoEndpointId] || EMPTY_LOOKUP_STATE;
  const currentParams = currentLookupState.params || {};
  const currentBindings = currentLookupState.fieldBindings || {};
  const currentBodyText = currentLookupState.bodyText || '';
  const currentError = currentLookupState.error;
  const currentLoading = currentLookupState.loading;
  const currentMappedFields = currentLookupState.mappedFields || [];
  const currentResponse = currentLookupState.response;

  function renderInfoLookupPanel() {
    if (inline) return null;
    if (infoEndpointList.length === 0) return null;
    const available = infoEndpointDefinitions.filter((def) => def && def.id);
    return (
      <div className="border border-gray-200 rounded bg-gray-50 p-3 space-y-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="m-0 font-semibold">{t('posapi_lookup_title', 'Information lookups')}</h3>
            <p className="m-0 text-sm text-gray-600">
              {t(
                'posapi_lookup_hint',
                'Call POSAPI information endpoints to verify customer or tax data.',
              )}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <select
              ...
              value={selectedInfoEndpointId}
              onChange={(e) => setSelectedInfoEndpointId(e.target.value)}
            >
            ...
            <button
              ...
              disabled={!selectedInfoEndpointId || currentLoading}
            >
              {currentLoading
                ? t('posapi_lookup_invoking', 'Invoking…')
                : t('posapi_lookup_invoke', 'Invoke')}
            </button>
          </div>
        </div>
        {infoEndpointsLoading && available.length === 0 ? (
          <div className="text-sm text-gray-600">
            {t('posapi_lookup_loading', 'Loading endpoint metadata…')}
          </div>
        ) : selectedDefinition ? (
          <div className="space-y-3">
            {Array.isArray(selectedDefinition.parameters) &&
            selectedDefinition.parameters.length > 0 ? (
              <div className="space-y-2">
                {selectedDefinition.parameters.map((param) => {
                  ...
                  const bindingValue = currentBindings[name] || '';
                  return (
                    <div
                      key={`info-param-${name}`}
                      className="grid gap-2 sm:grid-cols-2 sm:items-start"
                    >
                      <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700" htmlFor={`info-param-${name}`}>
                          {name}
                          {param.required ? ' *' : ''}
                        </label>
                        <input
                          ...
                          onChange={(e) => updateParamValue(selectedInfoEndpointId, name, e.target.value)}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700" htmlFor={`info-param-bind-${name}`}>
                          {t('posapi_lookup_bind_field', 'Bind to field')}
                        </label>
                        <select
                          id={`info-param-bind-${name}`}
                          className="border border-gray-300 rounded px-2 py-1 text-sm"
                          value={bindingValue}
                          onChange={(e) => updateParamBinding(selectedInfoEndpointId, name, e.target.value)}
                        >
                          <option value="">{t('posapi_lookup_no_binding', 'No binding')}</option>
                          {columnOptions.map((opt) => (
                            <option key={`info-param-opt-${name}-${opt.value}`} value={opt.value}>
                              {opt.label}
                            </option>
                          ))}
                        </select>
              ...
              <div className="text-sm text-gray-600">
                {t('posapi_lookup_no_params', 'This endpoint does not require parameters.')}
              </div>
            )}
            {(() => {
              const method = (selectedDefinition.method || 'GET').toUpperCase();
              const needsBody = method !== 'GET' && method !== 'HEAD';
              if (!needsBody) return null;
              return (
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-medium text-gray-700" htmlFor="info-body">
                    {t('posapi_lookup_body', 'Request body (JSON)')}
                  </label>
                  <textarea
                    id="info-body"
                    rows={Math.max(3, currentBodyText.split(/\r?\n/).length || 3)}
                    className="border border-gray-300 rounded px-2 py-1 text-sm font-mono"
                    value={currentBodyText}
                    onChange={(e) => updateBodyText(selectedInfoEndpointId, e.target.value)}
                    placeholder={t('posapi_lookup_body_placeholder', '{ "key": "value" }')}
                  />
                </div>
              );
            })()}
            ...
            {currentMappedFields.length > 0 && (
              <div className="text-sm text-green-700">
                {t('posapi_lookup_mapped', 'Updated fields: {{fields}}', {
                  fields: currentMappedFields.map((entry) => entry.column).join(', '),
                })}
              </div>
            )}
            {currentResponse !== null && (
              <details className="text-sm" open>
                <summary className="cursor-pointer text-gray-700">
                  {t('posapi_lookup_response', 'Last response')}
                </summary>
                <pre className="mt-2 max-h-60 overflow-auto bg-white border border-gray-200 rounded p-2 text-xs whitespace-pre-wrap">
                  {formatLookupResponse(currentResponse)}
                </pre>
              </details>
            )}
          </div>
          ...
          <div className="text-sm text-gray-600">
            {t('posapi_lookup_select', 'Select a lookup endpoint to begin.')}
          </div>
        )}
      </div>
    );
