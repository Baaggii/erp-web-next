...
const RowFormModal = function RowFormModal({
  ...
  const recordIdCandidate = React.useMemo(() => {
  ...
  const infoEndpointList = React.useMemo(() => {
    if (!Array.isArray(infoEndpoints)) return [];
    const seen = new Set();
    const result = [];
    infoEndpoints.forEach((id) => {
  ...
  const infoEndpointsKey = React.useMemo(() => infoEndpointList.join('|'), [infoEndpointList]);
  const [infoDefinitions, setInfoDefinitions] = useState([]);
  const [infoDefinitionsLoading, setInfoDefinitionsLoading] = useState(false);
  const [infoDefinitionsError, setInfoDefinitionsError] = useState('');
  const [selectedInfoEndpoint, setSelectedInfoEndpoint] = useState('');
  const [infoParamsByEndpoint, setInfoParamsByEndpoint] = useState({});
  const [infoRequestBodyByEndpoint, setInfoRequestBodyByEndpoint] = useState({});
  const [infoResponses, setInfoResponses] = useState({});
  const [infoInvokeLoading, setInfoInvokeLoading] = useState(false);
  const [infoInvokeError, setInfoInvokeError] = useState('');
  ...
  useEffect(() => {
    if (!infoEndpointList.length) {
      setInfoDefinitions([]);
      setSelectedInfoEndpoint('');
      setInfoDefinitionsError('');
      setInfoDefinitionsLoading(false);
      return;
    }
    const controller = new AbortController();
    setInfoDefinitionsLoading(true);
    setInfoDefinitionsError('');
    const params = new URLSearchParams();
    params.set('ids', infoEndpointList.join(','));
    fetch(`${API_BASE}/posapi/info?${params.toString()}`, {
      credentials: 'include',
      signal: controller.signal,
    })
      .then((res) => {
        if (!res.ok) {
          throw new Error(`Failed to load lookup endpoints (${res.status})`);
        }
        return res.json();
      })
      .then((data) => {
        if (controller.signal.aborted) return;
        setInfoDefinitions(Array.isArray(data) ? data : []);
      })
      .catch((err) => {
        if (controller.signal.aborted) return;
        setInfoDefinitions([]);
        setInfoDefinitionsError(err?.message || 'Failed to load lookup endpoints');
      })
      .finally(() => {
  ...
  }, [API_BASE, infoEndpointsKey, infoEndpointList]);

  useEffect(() => {
    setInfoParamsByEndpoint((prev) => {
      let changed = false;
      const next = {};
      infoEndpointList.forEach((id) => {
    ...
    setInfoRequestBodyByEndpoint((prev) => {
      let changed = false;
      const next = {};
      infoEndpointList.forEach((id) => {
  ...
  }, [infoEndpointList, infoEndpointsKey]);

  useEffect(() => {
    if (!infoDefinitions.length) {
      setSelectedInfoEndpoint('');
      return;
    }
    setSelectedInfoEndpoint((prev) => {
      if (prev && infoDefinitions.some((def) => def?.id === prev)) {
        return prev;
      }
      const fallback = infoDefinitions[0]?.id || infoEndpointList[0] || '';
      return fallback || '';
    });
  }, [infoDefinitions, infoEndpointList]);

  useEffect(() => {
    if (!infoDefinitions.length) return;
    setInfoParamsByEndpoint((prev) => {
      let changed = false;
      const next = { ...prev };
      infoDefinitions.forEach((def) => {
        if (!def?.id) return;
        if (next[def.id]) return;
        const defaults = {};
        (Array.isArray(def.parameters) ? def.parameters : []).forEach((param) => {
          if (!param?.name) return;
          const candidate =
            param.testValue ?? param.example ?? param.default ?? param.sample ?? '';
          if (candidate === undefined || candidate === null) return;
          defaults[param.name] = String(candidate);
        });
        next[def.id] = defaults;
        changed = true;
      });
      return changed ? next : prev;
    });
  ...
  function tokenizePath(path) {
    ...
    while ((match = re.exec(path)) !== null) {
      const [, bare, doubleQuoted, singleQuoted] = match;
      let value = bare ?? doubleQuoted ?? singleQuoted;
      if (value === undefined) {
  ...
  function extractInfoValue(source, path) {
  ...
  const handleInfoParamChange = useCallback((endpointId, name, value) => {
    if (!endpointId || !name) return;
    setInfoParamsByEndpoint((prev) => {
      const existing = prev[endpointId] && typeof prev[endpointId] === 'object'
        ? prev[endpointId]
        : {};
      if (existing[name] === value) return prev;
      return {
        ...prev,
        [endpointId]: { ...existing, [name]: value },
      };
    });
  }, []);

  const handleInfoRequestBodyChange = useCallback((endpointId, value) => {
    if (!endpointId) return;
    setInfoRequestBodyByEndpoint((prev) => {
      if (prev[endpointId] === value) return prev;
      return { ...prev, [endpointId]: value };
    });
  }, []);

  const applyInfoMappingFromResponse = useCallback(
    (endpointId, payload) => {
      if (!endpointId) return;
      if (!payload || typeof payload !== 'object') return;
      if (!infoEndpointMappings || typeof infoEndpointMappings !== 'object') return;
      const mapping = infoEndpointMappings[endpointId];
      ...
      Object.entries(mapping).forEach(([field, path]) => {
        ...
        const value = extractInfoValue(payload, trimmedPath);
        if (value === undefined) return;
        const formatted = formatInfoValue(value);
        const lower = trimmedField.toLowerCase();
        const resolved = columnCaseMap[lower] || trimmedField;
        if (columnLowerSet.has(lower) || columnLowerSet.has(resolved.toLowerCase())) {
          formUpdates[resolved] = formatted;
        } else {
          extraUpdates[trimmedField] = formatted;
        }
      });
      ...
      if (Object.keys(formUpdates).length > 0) {
        ...
        if (diff) {
          Object.keys(diff).forEach((key) => updatedFields.add(key));
        }
      }
      if (Object.keys(extraUpdates).length > 0) {
        ...
        if (extraDiff && Object.keys(extraDiff).length > 0) {
          onChange(extraDiff);
          Object.keys(extraDiff).forEach((key) => updatedFields.add(key));
        }
      }
      if (updatedFields.size > 0) {
        window.dispatchEvent(
          new CustomEvent('toast', {
            detail: {
              message: `Updated fields: ${Array.from(updatedFields).join(', ')}`,
              type: 'info',
            },
          }),
        );
    ...
    [infoEndpointMappings, columnCaseMap, columnLowerSet, setFormValuesWithGenerated, setExtraVals, onChange],
  );

  const handleInvokeInfoEndpoint = useCallback(async () => {
    if (!selectedInfoEndpoint) {
      setInfoInvokeError('Select a lookup endpoint to invoke.');
      return;
    }
    const endpointId = selectedInfoEndpoint;
    const paramsMap = infoParamsByEndpoint[endpointId] || {};
    ...
    Object.entries(paramsMap).forEach(([key, value]) => {
      ...
      if (str === '') return;
      normalizedParams[key] = str;
    });
    const rawBody = infoRequestBodyByEndpoint[endpointId];
    let parsedBody;
    if (rawBody && rawBody.trim()) {
      try {
        parsedBody = JSON.parse(rawBody);
      } catch (err) {
        setInfoInvokeError('Request body must be valid JSON.');
        return;
      }
    }
    const contextPayload = {
      table,
      formName,
      recordId: recordIdCandidate ?? null,
    };
    setInfoInvokeLoading(true);
    setInfoInvokeError('');
    try {
      const res = await fetch(
        `${API_BASE}/posapi/info/${encodeURIComponent(endpointId)}/invoke`,
        {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ params: normalizedParams, body: parsedBody, context: contextPayload }),
        },
      );
      if (!res.ok) {
      ...
      setInfoResponses((prev) => ({ ...prev, [endpointId]: payload }));
      applyInfoMappingFromResponse(endpointId, payload);
      const endpointLabel = data?.endpoint?.name || endpointId;
      window.dispatchEvent(
        new CustomEvent('toast', {
          detail: { message: `Lookup completed for ${endpointLabel}`, type: 'info' },
        }),
      );
    } catch (err) {
      setInfoInvokeError(err?.message || 'Lookup failed');
    } finally {
      setInfoInvokeLoading(false);
    }
  }, [
    selectedInfoEndpoint,
    infoParamsByEndpoint,
    infoRequestBodyByEndpoint,
    table,
    formName,
    recordIdCandidate,
    API_BASE,
    applyInfoMappingFromResponse,
  ]);
  ...
  function renderInfoLookupSection() {
    if (infoEndpointList.length === 0) return null;
    const options = infoEndpointList.map((id) => {
      const definition = infoDefinitions.find((entry) => entry?.id === id) || null;
      const label = definition?.name ? `${id} – ${definition.name}` : id;
      return { id, label, method: definition?.method || 'GET', definition };
    });
    const hasSelected = options.some((opt) => opt.id === selectedInfoEndpoint);
    const activeOption = hasSelected ? options.find((opt) => opt.id === selectedInfoEndpoint) : options[0];
    const activeId = activeOption?.id || '';
    const definition = activeOption?.definition || null;
    const method = (definition?.method || 'GET').toUpperCase();
    const parameters = Array.isArray(definition?.parameters) ? definition.parameters : [];
    const paramValues = infoParamsByEndpoint[activeId] || {};
    const requestBodyValue = infoRequestBodyByEndpoint[activeId] ?? '';
    const responseValue = infoResponses[activeId];
    let responsePreview = '';
    if (responseValue !== undefined) {
      if (typeof responseValue === 'string') responsePreview = responseValue;
      else {
        try {
          responsePreview = JSON.stringify(responseValue, null, 2);
        } catch {
          responsePreview = String(responseValue);
        }
      }
    }
      ...
      >
        ...
        >
          <label style={{ display: 'flex', flexDirection: 'column', minWidth: '220px' }}>
            <span style={{ fontWeight: 600 }}>POSAPI lookup endpoint</span>
            <select
              value={activeId}
              onChange={(e) => setSelectedInfoEndpoint(e.target.value)}
              disabled={infoDefinitionsLoading}
            >
              {options.map((opt) => (
                <option key={`info-select-${opt.id}`} value={opt.id}>
                  {opt.label}
                </option>
              ))}
            </select>
          ...
          <button
            ...
            disabled={!activeId || infoInvokeLoading}
            style={{
              padding: '0.4rem 0.9rem',
              backgroundColor: infoInvokeLoading ? '#9ca3af' : '#2563eb',
              color: '#fff',
              border: 'none',
              borderRadius: '4px',
              cursor: !activeId || infoInvokeLoading ? 'not-allowed' : 'pointer',
            }}
          >
            {infoInvokeLoading ? 'Fetching…' : 'Fetch info'}
          </button>
        </div>
        {infoDefinitionsError && (
          <div style={{ color: '#b91c1c', marginBottom: '0.5rem' }}>{infoDefinitionsError}</div>
        )}
        {infoInvokeError && (
          <div style={{ color: '#b91c1c', marginBottom: '0.5rem' }}>{infoInvokeError}</div>
        )}
        {definition && parameters.length > 0 && (
          ...
          >
            {parameters.map((param) => (
              <label key={`${definition.id}-param-${param.name}`} style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <input
                  ...
                  onChange={(e) => handleInfoParamChange(definition.id, param.name, e.target.value)}
                  style={{ padding: '0.35rem 0.5rem', border: '1px solid #d1d5db', borderRadius: '4px' }}
                />
                {param.description && (
                  <small style={{ color: '#6b7280' }}>{param.description}</small>
                )}
              </label>
            ))}
          </div>
        ...
        {definition && method !== 'GET' && method !== 'HEAD' && (
          <label style={{ display: 'flex', flexDirection: 'column', marginBottom: '0.75rem' }}>
            <span style={{ fontWeight: 600 }}>Request body ({method})</span>
            <textarea
              value={requestBodyValue}
              onChange={(e) => handleInfoRequestBodyChange(definition.id, e.target.value)}
              placeholder={'{\n  "key": "value"\n}'}
              style={{
            ...
            />
            <small style={{ color: '#6b7280' }}>Provide JSON payload for POST/PUT lookups.</small>
          </label>
        )}
        {responsePreview && (
          <div>
