...
function buildThread(messages, rootId) {
  const scoped = sortByTimeAsc(
    messages.filter((m) => {
      const currentId = Number(m.id);
      const parentId = Number(m.parent_message_id);
      if (currentId === Number(rootId)) return true;
      return parentId > 0;
    }),
  );

  const byId = new Map(scoped.map((m) => [Number(m.id), { ...m, replies: [] }]));
  const roots = [];
  byId.forEach((node) => {
