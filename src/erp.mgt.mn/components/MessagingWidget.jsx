...
function buildThread(messages, rootId) {
  ...
  const resolveRootId = (messageId) => {
    if (rootMemo.has(messageId)) return rootMemo.get(messageId);
    ...
    while (current && current.parent_message_id && guard < 2000) {
      ...
      if (!byId.has(parentId)) {
        rootMemo.set(messageId, messageId);
        return messageId;
      }
      current = byId.get(parentId);
      guard += 1;
    }
    const resolved = Number(current?.id || messageId);
    rootMemo.set(messageId, resolved);
    return resolved;
  };
  ...
  byId.forEach((node, nodeId) => {
    if (resolveRootId(nodeId) === Number(rootId)) {
      inThread.push(node);
    }
  });

  // prune replies to this thread only and preserve chronological ordering
  const threadIds = new Set(inThread.map((node) => Number(node.id)));
  inThread.forEach((node) => {
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 4 }}>
                  {(selectedRecipients || []).map((empid) => (
                    <button key={empid} onClick={() => applyMention(empid)} style={{ fontSize: '0.68rem' }}>@{employees.find((emp) => String(emp.empid) === String(empid))?.name || empid}</button>
                  ))}
                </div>
