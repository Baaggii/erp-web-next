...
export default function MessagingWidget() {
  ...
  const closeOpenMessageMenus = useCallback(() => {
    document.querySelectorAll("section[aria-label='Messaging widget'] details[open]").forEach((node) => {
      node.open = false;
    });
  }, []);
  ...
  useEffect(() => {
    const onPointerDown = (event) => {
      const root = document.querySelector("section[aria-label='Messaging widget']");
      if (!root) return;
