...
export async function login({ empid, password, companyId }, t = (key, fallback) => fallback || key) {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      const message = tokenType.includes('application/json')
    ...
    if (!tokenType.includes('application/json')) {
      throw new Error(t('unableToReachServer', 'Unable to reach server'));
    }
  ...
  if (!res.ok) {
    ...
    if (dataType.includes('application/json')) {
      ...
      message = t('unableToReachServer', 'Unable to reach server');
    } else {
