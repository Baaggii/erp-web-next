...
test('getProcedureLockCandidates derives labels from snapshot rows', async () => {
  const queryLog = [];
  let released = false;
  const snapshotCalls = [];
  ...
  try {
    const candidates = await db.getProcedureLockCandidates(
      ...
      {
        companyId: 1,
        resolveSnapshotRow: async (table, recordId) => {
          snapshotCalls.push([table, recordId]);
          return snapshotRow;
        },
      },
    );
    ...
    assert.deepEqual(snapshotCalls[0], ['code_chiglel', '7']);
    const [candidate] = candidates;
    assert.equal(candidate.label, 'CH-01 — Retail');
    assert.equal(candidate.description, 'Retail');
    assert.deepEqual(candidate.snapshot, snapshotRow);
    assert.deepEqual(
      new Set(candidate.snapshotColumns || []),
      new Set(Object.keys(snapshotRow)),
    );
    assert.equal(candidate.context?.snapshot, snapshotRow);
    assert.ok(released, 'connection should be released');
    assert.ok(
      queryLog.some(([sql]) => sql.startsWith('CALL sp_snapshot_labels')),
      'stored procedure should be invoked',
    );
  } finally {
    restoreGetConnection();
