...
test('listTemporarySubmissions supports cursor pagination for merged chain rows', async () => {
  const captured = [];
  const now = new Date().toISOString();
  const row = {
    id: 301,
  ...
  const restore = mockQuery(async (sql, params = []) => {
    ...
    if (sql.startsWith('CREATE TABLE IF NOT EXISTS `transaction_temporaries`')) return [[], []];
    if (sql.startsWith('CREATE TABLE IF NOT EXISTS `transaction_temporary_review_history`')) return [[], []];
    if (sql.includes('INFORMATION_SCHEMA.COLUMNS')) return [[{ COLUMN_NAME: 'chain_id' }, { COLUMN_NAME: 'is_pending' }]];
    if (sql.includes('INFORMATION_SCHEMA.TABLE_CONSTRAINTS')) return [[{ CONSTRAINT_NAME: 'chk_temp_pending_reviewer' }]];
    if (sql.includes('INFORMATION_SCHEMA.STATISTICS')) return [[{ INDEX_NAME: 'idx_temp_chain_pending' }]];
    if (sql.includes('INFORMATION_SCHEMA.TRIGGERS')) return [[{ TRIGGER_NAME: 'trg_temp_clear_reviewer' }]];
    if (sql.startsWith('ALTER TABLE `transaction_temporaries`')) return [[], []];
    if (sql.startsWith('UPDATE `transaction_temporaries` SET chain_id = id WHERE chain_id IS NULL')) return [[], []];
    if (sql.startsWith('UPDATE `transaction_temporaries` SET chain_id = ? WHERE id = ?')) return [[], []];
    if (sql.includes('WITH filtered AS')) return [[row]];
    throw new Error(`Unexpected SQL: ${sql}`);
  });

  try {
    const cursor = Buffer.from(
      JSON.stringify({ updatedAt: '2024-01-06T00:00:00.000Z', id: 302 }),
    ).toString('base64');
    ...
    const query = captured.find(({ sql }) => sql.includes('WITH filtered AS'));
    assert.ok(query);
    assert.ok(query.sql.includes('filtered.updated_at < ? OR (filtered.updated_at = ? AND filtered.id < ?)'));
