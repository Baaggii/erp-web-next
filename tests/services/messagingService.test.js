...
test('private replies keep delivery scoped to original participants', async () => {
  const db = new FakeDb();
  ...
  const root = await postMessage({
    ...
    payload: {
      ...
      visibilityEmpid: 'e-2',
      idempotencyKey: 'private-participants-root',
    },
    correlationId: 'private-participants-root',
    db,
    getSession: async () => session,
  });
  ...
  const reply = await postReply({
    ...
    payload: { body: 'private reply', idempotencyKey: 'private-participants-reply' },
    correlationId: 'private-participants-reply',
    db,
    getSession: async () => session,
  });
  ...
  const authorView = await getThread({
    ...
    companyId: 1,
    messageId: root.message.id,
    correlationId: 'private-participants-author-view',
    db,
    getSession: async () => session,
  });

  assert.equal(authorView.replies.length, 1);
  assert.equal(authorView.replies[0].id, reply.message.id);
});


test('private replies emit only to selected thread participants', async () => {
  const db = new FakeDb();
  const session = { permissions: { messaging: true }, department_id: 10 };
  const emissions = [];
  ...
  const root = await postMessage({
    ...
    payload: {
      ...
      visibilityScope: 'private',
      visibilityEmpid: 'e-2',
      idempotencyKey: 'private-emit-root',
    },
    correlationId: 'private-emit-root',
    db,
    getSession: async () => session,
  });
  ...
  await postReply({
    ...
    companyId: 1,
    messageId: root.message.id,
    payload: { body: 'private reply', idempotencyKey: 'private-emit-reply' },
    correlationId: 'private-emit-reply',
    db,
    getSession: async () => session,
  });

  const rooms = emissions.map((entry) => entry.room);
  assert.ok(rooms.includes('user:e-1'));
  assert.ok(rooms.includes('user:e-2'));
  assert.ok(!rooms.includes('user:e-3'));
});
