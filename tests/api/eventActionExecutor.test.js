...
test('call_procedure appends company scope parameter by default', async () => {
  const conn = makeConn();
  const policy = {
    action_json: {
      actions: [
        {
          type: 'call_procedure',
          procedure: 'sp_policy_demo',
          allowList: ['sp_policy_demo'],
          params: ['A', 10],
        },
      ],
    },
  ...
  const results = await executePolicyActions({
    event: { eventType: 'x', source: {} },
    policy,
    companyId: 44,
    conn,
  });

  assert.equal(results[0].type, 'call_procedure');
  const call = conn.calls.find((entry) => entry.sql.includes('CALL `sp_policy_demo`'));
  assert.ok(call);
  assert.deepEqual(call.params, ['A', 10, 44]);
});
