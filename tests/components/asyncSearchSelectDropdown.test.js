...
if (!haveReact) {
  ...
  test('AsyncSearchSelect eagerly loads additional search pages', { skip: true }, () => {});
} else {
  ...
  test('AsyncSearchSelect eagerly loads additional search pages', async (t) => {
    const restoreDom = setupDom();
    const origFetch = global.fetch;
    const requests = [];
      ...
      if (url.startsWith('/api/tables/items?')) {
        ...
        if (page === '2') {
          return {
            ...
            json: async () => ({
              rows: [
                { id: 75, code: 'ITM-75', name: 'Item-75' },
              ],
              count: 120,
            }),
          };
    ...
    const { default: AsyncSearchSelect } = await t.mock.import(
      ...
      {
        '../context/AuthContext.jsx': {
          AuthContext: React.createContext({ company: 1, branch: 1, department: 1 }),
        },
        '../utils/tenantKeys.js': { getTenantKeyList: () => [] },
        '../utils/buildAsyncSelectOptions.js': {
          buildOptionsForRows: async ({ rows, idField, labelFields }) =>
    ...
    const input = container.querySelector('input');
    assert.ok(input, 'input should be rendered for eager search test');
    ...
    const searchRequests = requests.filter((url) => url.includes('search=Item-75'));
    assert.equal(
      searchRequests.length,
      2,
      'should automatically request subsequent search pages',
    );
    const renderedItems = Array.from(container.querySelectorAll('li'));
    assert.ok(renderedItems.some((li) => li.textContent.includes('Item-75')));
