...
if (typeof mock?.import !== 'function') {
  ...
  test('TableRelationsEditor posts combination filters', async (t) => {
    const reactMock = createReactMock();
    const originalFetch = global.fetch;
    let savedBody = null;
    global.fetch = async (url, options = {}) => {
      const key = typeof url === 'string' ? url : url?.url;
      if (key === '/api/tables/bands/columns') {
        return { ok: true, json: async () => ['id', 'util_id', 'band_id'] };
      }
      if (key === '/api/tables') {
        return { ok: true, json: async () => ['bands', 'code_utility_rates'] };
      }
      if (key === '/api/tables/bands/relations') {
        return { ok: true, json: async () => [] };
      }
      if (key === '/api/tables/bands/relations/custom') {
        return { ok: true, json: async () => ({ relations: {}, isDefault: true }) };
      }
      if (key === '/api/tables/code_utility_rates/columns') {
        return { ok: true, json: async () => ['band_id', 'utility_id'] };
      }
      if (key === '/api/tables/bands/relations/custom/band_id') {
        savedBody = JSON.parse(options.body);
        return {
          ok: true,
          json: async () => ({
            column: 'band_id',
            relation: { table: 'code_utility_rates', column: 'band_id' },
            index: 0,
            relations: [
              { table: 'code_utility_rates', column: 'band_id' },
            ],
          }),
        };
      }
    ...
    reactMock.render(TableRelationsEditor, { table: 'bands' });
    await flushPromises();
    await flushPromises();

    reactMock
      .findByTestId('relations-column-select')
      .props.onChange({ target: { value: 'band_id' } });
    await flushPromises();

    await reactMock
      .findByTestId('relations-target-table')
      .props.onChange({ target: { value: 'code_utility_rates' } });
    await flushPromises();
    await flushPromises();

    reactMock
      .findByTestId('relations-target-column')
      .props.onChange({ target: { value: 'band_id' } });

    const addBtn = reactMock.findByTestId('relations-combo-add');
    await addBtn.props.onClick();

    reactMock
      .findByTestId('relations-combo-source-0')
      .props.onChange({ target: { value: 'util_id' } });
    reactMock
      .findByTestId('relations-combo-target-0')
      .props.onChange({ target: { value: 'utility_id' } });

    await reactMock.findByTestId('relations-save').props.onClick();
    await flushPromises();

    assert.ok(savedBody);
    assert.deepEqual(savedBody, {
      targetTable: 'code_utility_rates',
      targetColumn: 'band_id',
      combinationFields: [{ sourceField: 'util_id', targetField: 'utility_id' }],
    });
