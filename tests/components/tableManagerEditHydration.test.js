...
if (!haveReact) {
  ...
  test(
    'TableManager maps underscored tenant keys to camel case columns',
    { skip: true },
    () => {},
  );
  test(
  ...
  test('TableManager prefers row tenant keys over session defaults when editing', async (t) => {
    const prevWindow = global.window;
    const prevDocument = global.document;
    const prevNavigator = global.navigator;
    ...
    global.fetch = async (input) => {
      ...
      if (url.startsWith('/api/tables/test?')) {
        return {
          ...
          json: async () => ({
            rows: [
              { id: 1, COMPANY_ID: 99, branch_id: 88, name: 'Row 1' },
            ],
            count: 1,
          }),
        };
      ...
      if (url.startsWith('/api/tables/test/1')) {
        ...
        return {
          ok: true,
          json: async () => ({ id: 1, name: 'Row 1', company_id: 99, branch_id: 88 }),
        };
      }
      return { ok: true, json: async () => ({}) };
    };
    ...
    try {
      ...
      const lastProps = modalProps.at(-1);
      assert.ok(lastProps?.visible, 'expected modal to be visible');
      assert.equal(lastProps.row?.company_id, 99);
      assert.equal(lastProps.rows?.[0]?.branch_id, 88);
      assert.equal(toasts.length, 0, 'expected no error toasts');
      assert.ok(detailCalls.length >= 1, 'expected detail fetch to be called');
  ...
  test('TableManager honors camelCase tenant keys from the API', async (t) => {
    ...
    try {
      ...
      assert.equal(lastProps.rows?.[0]?.BranchID, 42);
      assert.equal(toasts.length, 0, 'expected no error toasts');
      assert.ok(detailCalls.length >= 1, 'expected detail fetch to be called');
  ...
  test('TableManager maps underscored tenant keys to camel case columns', async (t) => {
    ...
    global.fetch = async (input) => {
      ...
      if (url === '/api/tenant_tables/test') {
        return {
          ok: true,
          json: async () => ({ tenantKeys: ['company_id', 'branch_id'] }),
        };
      }
      if (url.startsWith('/api/tables/test?')) {
        return {
          ...
          json: async () => ({
            rows: [
              { ID: 1, CompanyID: 77, BranchID: 66, Name: 'Row 1' },
            ],
            count: 1,
          }),
        };
      ...
      if (url.startsWith('/api/tables/test/1')) {
        ...
        return {
          ok: true,
          json: async () => ({ ID: 1, CompanyID: 77, BranchID: 66, Name: 'Row 1' }),
        };
      }
      return { ok: true, json: async () => ({}) };
    };
    ...
    try {
      ...
      const lastProps = modalProps.at(-1);
      assert.ok(lastProps?.visible, 'expected modal to be visible');
      assert.equal(lastProps.row?.CompanyID, 77);
      assert.equal(lastProps.rows?.[0]?.BranchID, 66);
      assert.equal(toasts.length, 0, 'expected no error toasts');
      assert.ok(detailCalls.length >= 1, 'expected detail fetch to be called');
