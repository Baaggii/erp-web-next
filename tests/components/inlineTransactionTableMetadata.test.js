...
if (typeof mock?.import !== 'function') {
  ...
  test('InlineTransactionTable normalizes object combination values for AsyncSearchSelect filters', async () => {
    const reactMock = createReactMock();
    const originalFetch = global.fetch;
    global.fetch = async () => ({ ok: true, json: async () => ({}) });
    ...
    try {
      reactMock.render(InlineTransactionTable, {
        ...
        labels: { company_id: 'Company', dept_id: 'Dept' },
        rows: [
          { company_id: { value: 'COMP-9', label: 'Company 9' }, dept_id: '' },
        ],
        disabledFields: [],
        defaultValues: {},
        onRowsChange: () => {},
      ...
      const deptCall = selectCalls.find((props) => props.table === 'departments');
      assert.ok(deptCall, 'relation AsyncSearchSelect should render');
      assert.deepEqual(deptCall.filters, { company_id: 'COMP-9' });
    } finally {
      global.fetch = originalFetch;
    }
  });
  ...
  test('InlineTransactionTable filters relation select options when source value is an object', async () => {
    const reactMock = createReactMock();
    const originalFetch = global.fetch;
    global.fetch = async () => ({ ok: true, json: async () => ({}) });
    ...
    try {
      reactMock.render(InlineTransactionTable, {
        ...
        labels: { company_id: 'Company', dept_id: 'Dept' },
        rows: [
          { company_id: { value: 'COMP-1', label: 'Company 1' }, dept_id: '' },
        ],
        disabledFields: [],
        defaultValues: {},
        onRowsChange: () => {},
