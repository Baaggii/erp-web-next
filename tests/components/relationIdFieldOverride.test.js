...
if (!haveReact) {
  ...
  test('RowFormModal applies combination filters from non-editable source fields', async (t) => {
    const origFetch = global.fetch;
    global.fetch = async () => ({ ok: true, json: async () => ({}) });
      ...
      await act(async () => {
        root.render(
          React.createElement(RowFormModal, {
            ...
            labels: { dept_id: 'Dept' },
            fieldTypeMap: { dept_id: 'string' },
          }),
        );
      });
      await new Promise((resolve) => setTimeout(resolve, 0));
      assert.ok(selectProps[0]);
      assert.deepEqual(selectProps[0].filters, { company_id: 'COMP-2' });
    } finally {
      root.unmount();
      global.fetch = origFetch;
    }
  ...
  test('RowFormModal filters combination select options using extra values', async (t) => {
    const origFetch = global.fetch;
    global.fetch = async () => ({ ok: true, json: async () => ({}) });
