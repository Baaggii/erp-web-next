...
if (typeof mock.import !== 'function') {
  ...
  test('Reports emits workplace toast when toggle is provided at root level', async () => {
    const fetchCalls = [];
    global.fetch = async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url.startsWith('/api/report_procedures')) {
        return {
          ok: true,
          json: async () => ({ procedures: [{ name: 'report_with_sql' }] }),
        };
      }
      if (url.startsWith('/api/procedures/report_with_sql/params')) {
        return {
          ok: true,
          json: async () => ({ parameters: ['year', 'month'] }),
        };
      }
      if (url.startsWith('/api/reports/workplaces')) {
        return {
          ...
          json: async () => ({
            assignments: [],
            diagnostics: { formattedSql: 'SELECT 1', params: [] },
          }),
        };
      }
      return { ok: true, json: async () => ({}) };
    ...
    const { default: ReportsPage } = await mock.import(
      '../../src/erp.mgt.mn/pages/Reports.jsx',
      {
        ...
        '../hooks/useGeneralConfig.js': {
          default: () => ({ workplaceFetchToastEnabled: 'true' }),
        },
        '../hooks/useHeaderMappings.js': { default: () => ({}) },
        '../hooks/useButtonPerms.js': { default: () => ({}) },
        '../components/CustomDatePicker.jsx': {
    ...
    assert.ok(procedureSelect, 'Procedure select not found for root toggle test');

    procedureSelect.props.onChange({ target: { value: 'report_with_sql' } });
    ...
    const yearInput = collectNodes(
      tree,
      (node) => node.type === 'input' && node.props?.placeholder === 'year',
    )[0];
    assert.ok(yearInput, 'Year input missing for root toggle test');
    yearInput.props.onChange({ target: { value: '2025' } });
    ...
    const monthInput = collectNodes(
      tree,
      (node) => node.type === 'input' && node.props?.placeholder === 'month',
    )[0];
    assert.ok(monthInput, 'Month input missing for root toggle test');
    monthInput.props.onChange({ target: { value: '10' } });
    ...
    assert.ok(startToast, 'Start toast should be emitted when root toggle is true');

    const successToast = addToastCalls.find(
      (call) =>
        (call.type === 'info' || call.type === 'success') &&
        call.message.includes('Workplace fetch params'),
    );
    assert.ok(successToast, 'Success toast should be emitted with root toggle');

    assert.ok(
      fetchCalls.some(({ url }) => url.startsWith('/api/reports/workplaces')),
      'Workplace fetch should have been executed',
    );
