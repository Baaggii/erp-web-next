...
if (typeof mock.import !== 'function') {
  ...
  test('Reports refetches workplaces when date inputs change', async () => {
    const fetchCalls = [];
    global.fetch = async (url, options = {}) => {
      fetchCalls.push({ url, options });
      if (url.startsWith('/api/report_procedures')) {
      ...
      if (url.startsWith('/api/procedures/report_with_workplace_dates/params')) {
        return {
          ok: true,
          json: async () => ({
            parameters: ['StartDate', 'EndDate', 'WorkPlaceId'],
          }),
        };
      }
      if (url.startsWith('/api/reports/workplaces')) {
        ...
        const startDateParam =
          urlObj.searchParams.get('startDate') || urlObj.searchParams.get('date');
        const suffix = startDateParam || 'unknown';
        return {
          ok: true,
          json: async () => ({
            assignments: [
              {
                workplace_id: suffix.endsWith('02-01') ? '3' : '2',
                workplace_session_id: suffix.endsWith('02-01') ? '33' : '22',
                workplace_name: `Workplace for ${suffix}`,
              },
            ],
          }),
        };
    ...
    assert.ok(procedureSelect, 'Procedure select not found for refetch test');

    procedureSelect.props.onChange({
      target: { value: 'report_with_workplace_dates' },
    });
    ...
    const datePickers = collectNodes(
      tree,
      (node) => node.type === 'CustomDatePicker',
    );
    assert.equal(datePickers.length, 2, 'Expected both start and end date pickers');

    const startPicker = datePickers[0];
    const endPicker = datePickers[1];

    startPicker.props.onChange('2025-01-01');
    endPicker.props.onChange('2025-01-31');
    ...
    startPicker.props.onChange('2025-02-01');
    ...
    assert.equal(
      workplaceCalls.length,
      2,
      'Expected workplace fetch to run for each distinct date change',
    );
    assert.ok(
      /startDate=2025-01-01/.test(workplaceCalls[0].url),
      'Initial workplace fetch missing January start date',
    );
    assert.ok(
      /startDate=2025-02-01/.test(workplaceCalls[1].url),
      'Second workplace fetch missing February start date',
    );

    const janToast = addToastCalls.find(
      (call) =>
        call.message.includes('Fetching workplaces with params') &&
        call.message.includes('2025-01-01'),
    );
    assert.ok(janToast, 'January fetch toast missing');

    const febToast = addToastCalls.find(
      (call) =>
        call.message.includes('Fetching workplaces with params') &&
        call.message.includes('2025-02-01'),
    );
    assert.ok(febToast, 'February fetch toast missing');
