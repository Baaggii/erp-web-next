...
export async function getEmploymentSessions(empid, options = {}) {
  ...
  const sql = `WITH ranked_schedule AS (
    SELECT
      ...
      es.end_date,
      ROW_NUMBER() OVER (
        PARTITION BY es.emp_id, es.workplace_id
  ...
  SELECT
    ...
    DATE_FORMAT(e.assignment_start_date, '%Y-%m') AS workplace_month,
    e.assignment_start_date AS workplace_start_date,
    e.assignment_end_date AS workplace_end_date,
    e.assignment_pos_no AS pos_no,
  ...
  GROUP BY e.employment_company_id, company_name,
           ...
           workplace_month, e.assignment_start_date, e.assignment_end_date, e.assignment_pos_no,
